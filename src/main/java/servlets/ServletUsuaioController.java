...
public class ServletUsuaioController extends ServletGenericUtil {

	private static final long serialVersionUID = 1L;

	private DAOUsuarioRepository daoUsuarioRepository = new DAOUsuarioRepository();

	public ServletUsuaioController() {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) { 
				...
				request.getRequestDispatcher("principal/usuario.jsp").forward(request, response); // Delete com redirecionamento 
				...
				response.getWriter().write("Excluido com sucesso!"); // Com ajax a resposta vem dessa meneira
				...
				List<ModelLogin> dadosJsonUser =  daoUsuarioRepository.consultaUsuarioList(Pesquisar, super.getUserLogado(request)); // Faz retorno da  Lista 
			
				ObjectMapper mapper = new ObjectMapper();
		
				String json = mapper.writeValueAsString(dadosJsonUser);
				
				response.addHeader("totalPagina", "" + daoUsuarioRepository.consultaUsuarioListTotalPaginaPaginacao(Pesquisar ,super.getUserLogado(request)));
				
				response.getWriter().write(json); // Com json a resposta vem dessa meneira
 
			}
			
			else  if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("pesquisarUserAjaxPage")) {

				String Pesquisar = request.getParameter("Pesquisar");
				String pagina = request.getParameter("pagina");

				List<ModelLogin> dadosJsonUser =  daoUsuarioRepository.consultaUsuarioListOffSet(Pesquisar, super.getUserLogado(request), Integer.parseInt(pagina)); // Faz retorno da  Lista 
				...
				Integer offset = Integer.parseInt(request.getParameter("pagina")); // CONVERSÃO DO INTEGER
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			modelLogin.setId(id != null && !id.isEmpty() ? Long.parseLong(id) : null); // Conversão ternario
			...
			if (JakartaServletFileUpload.isMultipartContent(request)) {
				...
				if (part.getSize() > 0) { // SE REALMENTE PEGAR O TAMNHO DA FOTO CARREGA
